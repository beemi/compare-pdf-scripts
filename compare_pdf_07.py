...
# This script works for, script07 to launch ui

def create_gui_and_run_comparison():
    layout = [
        [sg.Text("PDF 1:"), sg.Input(), sg.FileBrowse(key="PDF1", file_types=(("PDF Files", "*.pdf"),))],
        [sg.Text("PDF 2:"), sg.Input(), sg.FileBrowse(key="PDF2", file_types=(("PDF Files", "*.pdf"),))],
        [sg.Text("Output PDF:"), sg.Input(), sg.FileSaveAs(key="Output", file_types=(("PDF Files", "*.pdf"),))],
        [sg.Button("Compare"), sg.Button("Cancel")]
    ]

    window = sg.Window("PDF Comparison Tool", layout)
    ...
    while True:
        ...
        if event == sg.WIN_CLOSED or event == "Cancel":
            break
        if event == "Compare":
            pdf_path1 = values["PDF1"]
            pdf_path2 = values["PDF2"]
            output_pdf_path = values["Output"]

            if not pdf_path1 or not pdf_path2 or not output_pdf_path:
                sg.popup_error("Please specify both PDF files and the output path.")
            else:
                try:
                    compare_pdfs_highlight_and_combine(pdf_path1, pdf_path2, output_pdf_path)
                    sg.popup("Success",
                             "The comparison is complete. The differences have been highlighted in the output PDF.")
                except Exception as e:
                    sg.popup_error("An error occurred:", str(e))
